...
- xsel (если планируете использовать только просмотрщик, используется в скрипте l_marcus.sh ,для его запуска, копируем md файл в буфер обмена и запускаем скрипт 
 l_marcus.sh, он запустит md файл в просмотрщике md, очень удобно станет если для этого скрипта назначить свою комбинацию горячих клавиш, у меня это ctrl+super+w)
...
## Использование
<u> Не забываем дать права на исполнение всем скриптам в директории проекта.</u>

Например:
```
chmod +x ./l_te-marcus.sh 
...
необходимо отредактировать под своего пользователя  путь откуда запускаетя `te_marcus_22-next.tcl`
...
- Предпросмотр и возможность экспорта в Html файл. (это обеспечивает вьювер для просмотра markdown, который как и описывалось выше может работать независимо от редактора.)
